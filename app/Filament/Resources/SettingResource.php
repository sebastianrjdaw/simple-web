...
class SettingResource extends Resource {
 ...
 private static function selectable(?Setting $record):bool{return in_array($record?->key,['playback_override','after_hours_mode','default_image_fit','default_transition_type','fallback_media_asset_id'],true);}
 public static function form(Form $form):Form{return $form->schema([
  ...
  Forms\Components\Select::make('value')->label('Valor')->options(fn(?Setting $record)=>match($record?->key){
   ...
   'default_transition_type'=>['fade'=>'Fundido','cut'=>'Corte'], 'fallback_media_asset_id'=>MediaAsset::where('media_type','image')->where('status','ready')->pluck('display_name','id')->all(), default=>[],
 ...
 public static function table(Table $table):Table{return $table->columns([Tables\Columns\TextColumn::make('key')->label('Ajuste')->formatStateUsing(fn($state)=>str_replace('_',' ',ucfirst($state))),Tables\Columns\TextColumn::make('value')->label('Valor')])->actions([Tables\Actions\EditAction::make()])->paginated(false);}
