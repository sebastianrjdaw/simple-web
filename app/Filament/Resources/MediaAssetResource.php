...
class MediaAssetResource extends Resource
{
    ...
    {
        return parent::getEloquentQuery()->where('status', '<>', 'duplicate');
    }

    public static function form(Form $form): Form
    {
        return $form->schema([
            Forms\Components\FileUpload::make('storage_path')->label('Archivos')->disk('media')->directory('originals')->storeFileNamesIn('original_filename')->multiple()->required()->acceptedFileTypes(['image/jpeg', 'image/png', 'image/webp', 'video/mp4'])->maxParallelUploads(2)->maxSize(self::maxUploadKilobytes())->helperText(self::uploadLimitText().' La validación y las miniaturas continuarán en segundo plano.')->visibleOn('create'),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            Tables\Columns\TextColumn::make('status')->label('Estado')->badge()->formatStateUsing(fn ($state) => match ($state) {
                ...
                'ready' => 'success','processing' => 'info','duplicate' => 'gray','error' => 'danger',default => 'gray'
            })->tooltip(fn (MediaAsset $record) => $record->validation_message),
    ...
    private static function maxUploadKilobytes(): int
    {
        ...
        $hard = max(1, (int) config('simpleview.max_upload_hard_mb'));

        return ($functional <= 0 ? $hard : min($functional, $hard)) * 1024;
    }

    private static function uploadLimitText(): string
    {
        $functional = (int) config('simpleview.max_upload_mb');
