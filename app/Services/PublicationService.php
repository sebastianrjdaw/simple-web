...
class PublicationService
{
    ...
    public function publish(Layout $draft, int $userId): Layout
    {
        ...
        if (! $this->storagePolicy->operationAllowed($storage)) {
            throw ValidationException::withMessages(['layout' => 'El almacenamiento está en estado crítico. Libera espacio antes de publicar.']);
        }
        ...
        return DB::transaction(function () use ($draft, $userId) {
            ...
            $published->fill(['state' => 'published', 'version' => $version, 'published_at' => now(), 'published_by' => $userId]);
            $published->snapshot_json = [];
            $published->save();
            foreach ($draft->zones as $zone) {
                $copy = $zone->replicate();
                $copy->layout_id = $published->id;
                $copy->save();
                foreach ($zone->items as $item) {
                    $ic = $item->replicate();
                    $ic->layout_zone_id = $copy->id;
                    $ic->save();
                }
            }
            $published->snapshot_json = $this->config($published);
            $published->save();
